...
def dump_cfg(cfg, show: bool=False):
    ...
    if cfg.DISTILLER.TYPE.startswith('VITKD'):
    ...
    if (distiller_type == 'VITKD') and cfg.VITKD.REF_SiNGER:
        dump.update({'SiNGER': cfg.get('SiNGER')})
...
# SiNGER
CFG.SiNGER= CN()
CFG.SiNGER.M_LAYERS = [5]  # manipulating layers
CFG.SiNGER.RANK = 16
CFG.SiNGER.NULL_THRES = 1.0E-3
CFG.SiNGER.OUTLIER_Q = 0.95
CFG.SiNGER.METHOD = 'singer' # 'SiNGER', 'RANDOM'
CFG.SiNGER.LOSS = CN()
CFG.SiNGER.LOSS.FEAT_WEIGHT = 100.0
CFG.SiNGER.LOSS.OUTLIER_WEIGHT = 0.01
CFG.SiNGER.LOSS.INFO_WEIGHT = 1.0
...
# FiTViT (Multi-layer fitnet for transformers) CFG
CFG.FITVIT = CN()
